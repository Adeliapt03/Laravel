...
<div class="container" data-aos="fade-up">
    ...
    <div class="section-title">
      ...
      <form name="formbio" action="/student/{{ $student->id }}" method="POST" onsubmit="return validateForm()">
      @csrf
      @method('put')
        ...
        <div class="form-group mb-4">
          ...
          <input type="text" class="form-control" name="nickname" value="{{ $student->nickname}}">
        </div>
  ...
  <select class="form-select" name="batchofstudent">
    <option selected>Select this option below</option>
    <option value="2020" @if ($student->batchofstudent == '2020') selected @endif>2020</option>
    <option value="2021" @if ($student->batchofstudent == '2021') selected @endif>2021</option>
    <option value="2022" @if ($student->batchofstudent == '2022') selected @endif>2022</option>
    <option value="2023" @if ($student->batchofstudent == '2023') selected @endif>2023</option>
  </select>
</div>

        <div>
          <strong><label>Gender :</label></strong>
          <p><input class="me-1" type='radio' name='gender' value='Male' @if($student->gender== 'Male'){{ "checked" }} @endif >Male</p>
          <p><input class="me-1" type='radio' name='gender' value='Female' @if($student->gender== 'Female'){{ "checked" }} @endif>Female</p>
        </div>

  <div class="form-group mb-3">
    <strong><label>Class Day :</label></strong>
    <input type="text" name="classday" id="classday" class="form-control" placeholder="classday" value="{{ $student->classday }}">
  </div>
  <div>
